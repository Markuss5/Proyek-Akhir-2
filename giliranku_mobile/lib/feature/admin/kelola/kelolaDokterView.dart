...
class _KelolaDokterViewState extends State<KelolaDokterView> {
  ...
  void _showFormDialog({Map<String, dynamic>? dokter}) {
    ...
    final namaController = TextEditingController(text: isEdit ? dokter['doctor_name'] : '');
    final spesialisasiController = TextEditingController(text: isEdit ? dokter['specialization'] : '');
    final noTelpController = TextEditingController(text: isEdit ? (dokter['phone'] ?? '') : '');
    final jadwalController = TextEditingController(text: isEdit ? (dokter['schedule'] ?? '') : '');
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<int>(
                  value: selectedPoly,
                  decoration: const InputDecoration(labelText: 'Poliklinik'),
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: 'Jadwal Praktik', hintText: 'Contoh: Senin, Selasa'),
                ),
              ],
            ),
          ),
          ...
          TextButton(
            onPressed: () async {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(success ? 'Dokter berhasil dihapus' : 'Gagal menghapus dokter'),
                    backgroundColor: success ? Colors.green : Colors.red,
                  ),
                );
              }
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    title: Text(item['doctor_name'] ?? '', style: const TextStyle(fontWeight: FontWeight.bold)),
                    subtitle: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const SizedBox(height: 4),
                        Text('${item['poly_name'] ?? ''} - ${item['specialization'] ?? ''}'),
                        if (item['schedule'] != null && item['schedule'].toString().isNotEmpty)
                    ...
                    trailing: Row(
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () => _showFormDialog(dokter: item),
                        ),
                        IconButton(
                          icon: const Icon(Icons.delete, color: Colors.red),
                          onPressed: () => _confirmDelete(item['doctor_id'], item['doctor_name']),
                        ),
                      ],
                    ),
                  ),
